...
public class clsGenero {
    ...
    public ResultSet listarGenero() throws Exception{
        strSQL="Select * from genero";
        try {
            rs=objCon.consultarBD(strSQL);
            return rs;
        } catch (Exception e) {
            throw new Exception("Error al listar genero");
        }
    }
    
    public String buscarPorId(int id) throws Exception{
        strSQL="Select nombre from genero where id_genero="+id;
        try {
            rs=objCon.consultarBD(strSQL);
            while(rs.next()){
    ...
    public int buscarPorNombre(String nom) throws Exception{
        strSQL="Select id_genero from genero where nombre='"+nom+"'";
        try {
            rs=objCon.consultarBD(strSQL);
            while(rs.next()){
